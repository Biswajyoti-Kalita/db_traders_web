    ...
    success: function (resultData) {
      ...
      for (var i = 0; i < result.length; i++) {
        $("#brandTableBody").append(`
					...
					<td>${
            result[i] ? (result[i].image ? result[i].image : " ") : " "
...
function addBrand() {
  ...
  if ($("#addBrandImageInput").get().files[0])
    formData.append("image", $("#addBrandImageInput").get().files[0], "image");
  formData.append("name", $("#addBrandNameInput").val(), "name");
  $.ajax({
    url: "/admin/addbrand",
    method: "POST",
    headers: {
    ...
    data: {
      name: $("#addBrandNameInput").val(),
      image: $("#addBrandImageInput").get().files[0],
    },
