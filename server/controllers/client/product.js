...
const role = 1;
					app.post("/client/addproduct",roleService.verifyRole(role),   async function(req, res) {
					    try {
							  ...
							  if(req.body.barcode === null || req.body.barcode === undefined)
								  return res.send({status  : "error", message : " Barcode is required " })
							  ...
							  if(req.body.colors === null || req.body.colors === undefined)
								  return res.send({status  : "error", message : " Colors is required " })
							  ...
							  if(req.body.description === null || req.body.description === undefined)
								  return res.send({status  : "error", message : " Description is required " })
			...
			description : req.body.description,
					...
					app.post("/client/updateproduct",  roleService.verifyRole(role),  async function(req, res) {
					    try {
			...
			description : req.body.description,
					...
					app.post("/client/deleteproduct",  roleService.verifyRole(role),  async function(req, res) {
					...
					app.post("/client/bulkdeleteproduct", roleService.verifyRole(role),  async function(req, res) {
				...
				app.post("/client/getproducts",  roleService.verifyRole(role),  async function(req, res) {
	...
	;
				...
				app.post("/client/getproduct",  roleService.verifyRole(role),  async function(req, res) {
				    try {
				        const product = await db.product.findOne({
				        	...
				        	attributes: ['id','uuid','barcode','name','tags','colors','cost_price','selling_price','tax','cgst','sgst','igst','discount','description']
				        });
				        res.send(product);
				    } catch (error) {
				        console.log(error);
