  ...
  $.ajax({
    ...
    success: function (result) {
      if (result) {
        ...
        console.log(item);
        var temp = parseFloat(item.selling_price);
        ...
        $("#imageDesc").html(`
             <div class="show" href="${
               ...
               <img src="${
                 item.images ? item.images[0].image_url : ""
               }" id="show-img" onclick="showBigImage()">
             </div>
             <div class="small-img">
        ...
        $("#productDesc").html(`
            <h4>${item.name}</h4>
            <h6>${item.brand ? item.brand.name : ""}</h6>

            <h2>₹${temp} <del>₹${item.selling_price}</del> <code>${
          item.discount
        }% off</code></h2>
            <br>
