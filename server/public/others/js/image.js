			...
			function updateImagesTableHeaderSort(){
				...
				$("#"+ImageTableOrderField+"Sort"+ImageTableOrderFieldBy).removeClass("fade-l");
			}
			...
			function getImages(searchObj) {
				    ...
				    success: function (resultData) {
				      ...
				      for (var i = 0; i < result.length; i++) {
				        $("#imageTableBody").append(`
				          <tr>
				          	<td> <input onclick="checkSelected('Image')" type="checkbox" class=" checkbox-Image "  data-id="${result[i].id}" /> </td>
					<td>${ result[i] ? result[i].id ? result[i].id : ' '  : ' '  }</td>
					<td>${ result[i] ? result[i].image_url ? result[i].image_url : ' '  : ' '  }</td>
					<td>${ result[i] ? result[i].size ? result[i].size : ' '  : ' '  }</td>
					<td>${ result[i] ? result[i].product_id ? result[i].product_id : ' '  : ' '  }</td><td><span class="btn btn-link btn-sm" onclick="editImageModal(  '${result[i].image_url}', '${result[i].size}', '${result[i].product_id}', ${result[i].id} )">Edit</span><span class="btn btn-link btn-sm" onclick="deleteImageModal(${result[i].id})">Delete</span></td>
				          </tr>
				        `);
				      }
				      getPaginate(count,changeImagesTableOffset,ImageTableLimit,ImageTableOffset,'Image')
				    },
				  });
				}
				...
				$("#searchImageForm").on('submit',(ev) => {
					...
					tempForm = ev;
					var searchObj ={};
					$("#searchImageForm").serializeArray().map((i) => {
						if(i.value)
				...
				function addImage() {
				  $.ajax({
				    ...
				    method: "POST",
				    data: {
				    	image_url :  $("#addImageImageUrlInput").val() ,size :  $("#addImageSizeInput").val() ,product_id :  $("#addImageProductIdInput").val() ,
				      token: Cookies.get("token"),
				    },
				...
				function updateImage()  {
				  $.ajax({
				    url: "/others/updateimage",
				    method: "POST",
				    data: {
				    	image_url : $("#editImageImageUrlInput").val(),size : $("#editImageSizeInput").val(),product_id : $("#editImageProductIdInput").val(),id : $("#editImageImageId").val(),				    
				      token: Cookies.get("token"),
				    },
				    success: function (result) {
				      console.log(result);
				...
				function editImageModal(image_url,size,product_id,id) {
				  ...
				  $("#editImageImageUrlInput").val(image_url);$("#editImageSizeInput").val(size);$("#editImageProductIdInput").val(product_id);
				}
