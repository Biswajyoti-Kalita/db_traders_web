			...
			function updateBrandsTableHeaderSort(){
				...
				$("#"+BrandTableOrderField+"Sort"+BrandTableOrderFieldBy).removeClass("fade-l");
			}
			...
			function getBrands(searchObj) {
				    ...
				    success: function (resultData) {
				      ...
				      for (var i = 0; i < result.length; i++) {
				        $("#brandTableBody").append(`
				          <tr>
				          	<td> <input onclick="checkSelected('Brand')" type="checkbox" class=" checkbox-Brand "  data-id="${result[i].id}" /> </td>
					<td>${ result[i] ? result[i].id ? result[i].id : ' '  : ' '  }</td>
					<td>${ result[i] ? result[i].name ? result[i].name : ' '  : ' '  }</td>
					<td>${ result[i] ? result[i].image ? result[i].image : ' '  : ' '  }</td><td><span class="btn btn-link btn-sm" onclick="editBrandModal(  '${result[i].name}', '${result[i].image}', ${result[i].id} )">Edit</span><span class="btn btn-link btn-sm" onclick="deleteBrandModal(${result[i].id})">Delete</span></td>
				          </tr>
				        `);
				      }
				      getPaginate(count,changeBrandsTableOffset,BrandTableLimit,BrandTableOffset,'Brand')
				    },
				  });
				}
				...
				$("#searchBrandForm").on('submit',(ev) => {
					...
					tempForm = ev;
					var searchObj ={};
					$("#searchBrandForm").serializeArray().map((i) => {
						if(i.value)
				...
				function addBrand() {
				  $.ajax({
				    ...
				    method: "POST",
				    data: {
				    	name :  $("#addBrandNameInput").val() ,image :  $("#addBrandImageInput").val() ,
				      token: Cookies.get("token"),
				    },
				...
				function updateBrand()  {
				  $.ajax({
				    url: "/others/updatebrand",
				    method: "POST",
				    data: {
				    	name : $("#editBrandNameInput").val(),image : $("#editBrandImageInput").val(),id : $("#editBrandBrandId").val(),				    
				      token: Cookies.get("token"),
				    },
				    success: function (result) {
				      console.log(result);
				...
				function editBrandModal(name,image,id) {
				  ...
				  $("#editBrandNameInput").val(name);$("#editBrandImageInput").val(image);
				}
