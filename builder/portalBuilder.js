...
function createMenus(menus) {
  ...
  menus.map((item, ind) => {
    data += `
	      	<li class="nav-item">
	            <a href="#menu_${item.controller_name}" class="nav-link ${
      ind == 0 ? "active" : ""
    }">
	              <i class="nav-icon ${
...
function createMenuDiv(current_portal, current_model) {
  var data = `
	`;
...
function createAddModals(menus, models) {
  var data = "";
  menus.map((item) => {
    if (item.add)
      data += `
...
function createEditModals(menus, models) {
  var data = "";
  menus.map((item) => {
    if (item.edit)
      data += `
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <!-- Main content -->
    <div class="content">
      ${createMenuDiv(current_portal, config_models)}
...
${createAddModals(current_portal.menus, config_models)}


${createEditModals(current_portal.menus, config_models)}
...
<script src="./js/script.js"></script>
	...
	${addMenuScripts(current_portal.menus)}
